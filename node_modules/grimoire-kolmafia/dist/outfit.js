import { bjornifyFamiliar, booleanModifier, canEquip, cliExecute, enthroneFamiliar, equip, equippedItem, Familiar, haveEquipped, Item, logprint, equippedAmount as mafiaEquippedAmount, weaponHands as mafiaWeaponHands, myBjornedFamiliar, myEnthronedFamiliar, myFamiliar, toSlot, useFamiliar, weaponType, } from "kolmafia";
import { $familiar, $familiars, $item, $skill, $slot, $slots, $stat, applyModes, get, have, Requirement, } from "libram";
...
export const riderSlots = ["buddy-bjorn", "crown-of-thrones"];
const weaponHands = (i) => (i ? mafiaWeaponHands(i) : 0);
...
export class Outfit {
    ...
    static current() {
        ...
        for (const slotName of outfitSlots) {
            const slot = (_a = new Map([
                ["famequip", $slot `familiar`],
                ["offhand", $slot `off-hand`],
            ]).get(slotName)) !== null && _a !== void 0 ? _a : toSlot(slotName);
    ...
    equipNonAccessory(item, slot) {
        if ($slots `acc1, acc2, acc3`.includes(toSlot(item)))
            return false;
        if (slot !== undefined && slot !== toSlot(item))
            return false;
        if (this.equips.has(toSlot(item)))
            return false;
        switch (toSlot(item)) {
            case $slot `off-hand`:
            ...
            case $slot `familiar`:
                ...
                break;
            case $slot `weapon`:
                if (!weaponsCompatible(item, this.equips.get($slot `off-hand`)))
                    return false;
                break;
        }
        if (toSlot(item) !== $slot `familiar` && !canEquip(item))
            return false;
        this.equips.set(toSlot(item), item);
        return true;
    }
    equipAccessory(item, slot) {
        if (![undefined, ...$slots `acc1, acc2, acc3`].includes(slot))
            return false;
    ...
    equipUsingDualWield(item, slot) {
        if (![undefined, $slot `off-hand`].includes(slot))
            return false;
        if (toSlot(item) !== $slot `weapon`)
            return false;
        if (this.equips.has($slot `weapon`) && weaponHands(this.equips.get($slot `weapon`)) !== 1) {
            return false;
        }
        ...
        if (!canEquip(item))
            return false;
        if (!weaponsCompatible(this.equips.get($slot `weapon`), item))
            return false;
        this.equips.set($slot `off-hand`, item);
        return true;
    }
    getHoldingFamiliar(item) {
        switch (toSlot(item)) {
            case $slot `weapon`:
    ...
    equipUsingFamiliar(item, slot) {
        if (![undefined, $slot `familiar`].includes(slot))
            return false;
        ...
        if (!this.equip(familiar))
            return false;
        this.equips.set($slot `familiar`, item);
        return true;
    }
    equipItem(item, slot) {
        return (this.haveEquipped(item, slot) ||
    ...
    equipSpec(spec) {
        ...
        for (const slotName of outfitSlots) {
            ...
            if (itemOrItems !== undefined && !this.equip(itemOrItems, slot))
                succeeded = false;
        }
        for (const item of (_b = spec === null || spec === void 0 ? void 0 : spec.equip) !== null && _b !== void 0 ? _b : []) {
            if (!this.equip(item))
                succeeded = false;
        }
        ...
        this.avoid.push(...((_c = spec === null || spec === void 0 ? void 0 : spec.avoid) !== null && _c !== void 0 ? _c : []));
        ...
        this.beforeDress(...((_e = spec.beforeDress) !== null && _e !== void 0 ? _e : []));
        this.afterDress(...((_f = spec.afterDress) !== null && _f !== void 0 ? _f : []));
        return succeeded;
    }
    /**
    ...
    static from(spec, error = null) {
        ...
        if (spec instanceof Requirement) {
            ...
            const cleanedResult = Object.fromEntries([...Object.entries(result)].filter(([, v]) => v !== undefined));
    ...
    setModes(modes) {
        ...
        if (this.modes["retrocape"] && modes["retrocape"]) {
            if (this.modes["retrocape"][0] &&
                modes["retrocape"][0] &&
                this.modes["retrocape"][0] !== modes["retrocape"][0]) {
                compatible = false;
            }
            if (this.modes["retrocape"][1] &&
                modes["retrocape"][1] &&
    ...
    _dress(refreshed) {
        ...
        for (const slot of nonaccessorySlots) {
            if ((targetEquipment.includes(equippedItem(slot)) &&
        ...
        for (const accessory of accessoryEquips) {
            const alreadyEquipped = accessorySlots.find((slot) => !usedSlots.has(slot) && equippedItem(slot) === accessory);
            if (alreadyEquipped) {
        ...
        if (bjorn && haveEquipped($item `Buddy Bjorn`)) {
            if (myEnthronedFamiliar() === bjorn)
                enthroneFamiliar($familiar.none);
            if (myBjornedFamiliar() !== bjorn)
                bjornifyFamiliar(bjorn);
        }
        if (crown && haveEquipped($item `Crown of Thrones`)) {
            if (myBjornedFamiliar() === crown)
                bjornifyFamiliar($familiar.none);
            if (myEnthronedFamiliar() !== crown)
                enthroneFamiliar(crown);
        }
    ...
    spec() {
        ...
        for (const slotName of outfitSlots) {
            const entry = this.equips.get((_a = new Map([
                ["famequip", $slot `familiar`],
                ["offhand", $slot `off-hand`],
            ]).get(slotName)) !== null && _a !== void 0 ? _a : toSlot(slotName));
            if (entry)
 ...
 */
export function convertToLibramModes(modes) {
    var _a;
    return {
...
export function convertFromLibramModes(modes) {
    return (modes.retrocape ? { ...modes, retrocape: modes.retrocape.split(" ") } : modes);
}
/**
...
function weaponsCompatible(weapon, offhand) {
    if (!weapon || weapon === $item `none`)
        return true;
    if (!offhand || offhand === $item `none`)
        return true;
    if (toSlot(offhand) !== $slot `weapon`)
        return true;
    const weaponStat = weaponType(weapon);
    const offhandStat = weaponType(offhand);
    if (weaponStat === $stat `Moxie` &&
        (offhandStat === $stat `Mysticality` || offhandStat === $stat `Muscle`))
        return false;
    if (offhandStat === $stat `Moxie` &&

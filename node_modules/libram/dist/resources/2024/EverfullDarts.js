...
function makePerkFunction(perkOrPerks, formula) {
    ...
    if (Array.isArray(perkOrPerks)) {
        return () => formula(perkOrPerks.filter((perk) => current.includes(perk)).length);
    }
 ...
 */
export const bullseyeChance = makePerkFunction(BULLSEYE_ACCURACY_PERKS, (perks) => clamp(0.25 * (1 + perks), 0, 1));
 ...
 */
export const bullseyeCooldown = makePerkFunction(BULLSEYE_COOLDOWN_PERKS, (perks) => clamp(50 - 10 * perks, 30, 50));
/**
 * @returns The total number of darts you can have available based on your current perks
 */
export const dartCapacity = makePerkFunction("Expand your dart capacity by 1", (perks) => 1 + perks);

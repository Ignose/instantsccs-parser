import { canAdventure, canInteract, cliExecute, descToItem, Location, myPath, Path, visitUrl, } from "kolmafia";
import { tuple } from "../../utils.js";
/**
 * @returns Whether the player can access the raffle house.
 */
export function accessible() {
    return (canAdventure(Location.get("South of the Border")) &&
        myPath() !== Path.get("Zombie Slayer"));
}
function getRafflePrize(place, page) {
    const descId = new RegExp(`${place} Prize:</td><td><img class=hand onclick='descitem\\((\\d+)\\)`).exec(page);
...
export function currentTickets() {
    ...
    if (page.includes("You haven't bought any tickets for today's raffle."))
        return 0;
    return Number(/You have purchased ([\d,]+) tickets for today's raffle/
        .exec(page)?.[1]
        .replace(/,/g, "") ?? 0);
}
/**
 ...
 */
export function buyTicketsUpTo(quantity) {

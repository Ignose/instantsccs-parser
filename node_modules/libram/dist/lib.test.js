...
vi.mocked(getPlayerId).mockImplementation((name) => {
...
describe(getPlayerIdFromName, () => {
    describe("with onMissing = 'throw'", () => {
        ...
        it.each([
            ...
            ["bufFy", 1889009],
            ["BuffY", 1889009],
        ])("should return the player id for a given name, case-insensitively", (name, id) => {
            expect(getPlayerIdFromName(name)).toBe(id);
        });
        it("should throw an error for an unknown name", () => {
            expect(() => getPlayerIdFromName("Unknown")).toThrowError("Player not found: Unknown");
        });
    });
...
describe(getPlayerNameFromId, () => {
    describe("with onMissing = 'throw'", () => {
        ...
        it("should throw an error for an unknown id", () => {
            expect(() => getPlayerNameFromId(123)).toThrowError("Player not found: 123");
        });
    });
    describe("with onMissing = 'return-null'", () => {
        it.each([
...
describe(getPlayerFromIdOrName, () => {
    describe("with onMissing = 'throw'", () => {
        ...
        it.each([
            ...
            ["bufFy", 1889009, "Buffy"],
            ["BuffY", 1889009, "Buffy"],
        ])("should return the player id and name for a given name, case-insensitively", (name, id, expectedName) => {
            expect(getPlayerFromIdOrName(name)).toEqual({ id, name: expectedName });
        });
        it.each(["Unknown", 123])("should throw an error for an unknown id or name", (idOrName) => {
            expect(() => getPlayerFromIdOrName(idOrName)).toThrowError(`Player not found: ${idOrName}`);
        });
    });
    describe("with onMissing = 'return-null'", () => {
        it.each([
        ...
        it.each(["Unknown", 123])("should return undefined for an unknown id or name", (idOrName) => {

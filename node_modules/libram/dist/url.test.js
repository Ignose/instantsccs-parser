...
describe(buildUrl, () => {
    ...
    ])("should append query (as object) to the given path", (path, query, expected) => {
    ...
    ])("should append query (as array) to the given path", (path, query, expected) => {
        expect(buildUrl(path, query)).toEqual(expected);
    });
    it.each([
    ...
    ])("should append a parameter even if it already exists", (path, query, expected) => {
        expect(buildUrl(path, query)).toEqual(expected);
    });
    it.each([
        [
    ...
    ])("should correctly encode non-string types", (path, query, expected) => {
        expect(buildUrl(path, query)).toEqual(expected);
    });
    it.each([
        [["foo", "bar", "baz"]],
...
describe(fetchUrl, () => {
    ...
    it("should call visitUrl with post=false if requested", () => {
        ...
        expect(visitUrl).toHaveBeenCalledWith("test.php?foo=bar%26baz", false, true);
    });
});
describe(combineQuery, () => {
    ...
    it("should combine more than two queries", () => {
        expect(combineQuery({ foo: "bar" }, [["baz", "qux"]], { quux: "quuz" })).toEqual([
            ["foo", "bar"],
            ["baz", "qux"],
            ["quux", "quuz"],
        ]);

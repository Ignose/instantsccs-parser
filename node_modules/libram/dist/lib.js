 ...
 */
export function getFoldGroup(item, cache = true) {
    if (cache) {
...
const todayOrAlways = (x) => get(`_${x}Today`) || get(`${x}Always`);
/**
...
function makeScalerCalcFunction(cache, pattern) {
    return function (monster) {
        ...
        const result = (pattern.exec(monster.attributes)?.slice(1) ?? []).find((m) => m !== undefined) ?? "0";
...
export function totalFamiliarWeight(familiar = myFamiliar(), considerAdjustment = true) {
    return clamp(clamp(familiarWeight(familiar), have($effect `Fidoxene`) ? 20 : 0, Infinity) +
        familiar.soupWeight +
...
export function getAcquirePrice(item, quantity = 1) {
    ...
    if (item.tradeable) {
        if (mallPrice(item) === mallMinPrice) {
            ...
            return (clamp(currentAmount, 0, quantity) * autosellPrice(item) +
